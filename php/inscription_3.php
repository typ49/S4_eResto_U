...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erreurs)) {
        ...
        $hash = password_hash($passe1, PASSWORD_DEFAULT);
        $naissance = str_replace('-','',$naissance);
        $sql_user = "INSERT INTO usager (usNom, usPrenom, usMail, usLogin, usPasse, usDateNaissance) VALUES ('$nom', '$prenom', '$email', '$login', '$hash', '$naissance')";
        $req_user = bdSendRequest($bd, $sql_user);
        affEntete("Vérification des données et enregistrement d'un nouvel utilisateur dans la base de données");
        affNav();
        echo 'un nouvel usager a bien été enregistré dans la base de données.';
    }
    else {
        $affErr = '<ul>';
